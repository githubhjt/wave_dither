...
float pattern(vec2 p) {
  float flow = p.y + time * waveSpeed;
  float xWarp = cnoise(vec2(p.x * 2.0, flow * 1.5)) * 0.15;
  float wave1 = sin(flow * 6.2831 * 1.0 + xWarp * 6.2831) * 0.5 + 0.5;
  float wave2 = sin(flow * 6.2831 * 2.3 + xWarp * 6.2831 * 1.5 + 1.0) * 0.3 + 0.5;
  float wave3 = sin(flow * 6.2831 * 0.7 + cnoise(vec2(p.x * 1.5, flow * 0.8)) * 3.0) * 0.4 + 0.5;
  float foam = fbm(vec2(p.x * 1.5, flow * 3.0)) * 0.4;
  float f = wave1 * 0.4 + wave2 * 0.25 + wave3 * 0.2 + foam * 0.35;
  f = smoothstep(0.3, 0.85, f);
  f = 1.0 - f;
  return f;
}
...
vec3 dither(vec2 uv, vec3 color) {
  ...
  // checker: 0 on even pixels, 1 on odd pixels
  float checker = mod(scaledCoord.x + scaledCoord.y, 2.0);
  // -0.25 or +0.25 → gray<0.25 all black, 0.25~0.75 checkerboard, gray>0.75 all white
  float threshold = checker * 0.5 - 0.25;
  float result = floor(clamp(gray + threshold, 0.0, 1.0) + 0.5);
  return vec3(result);
}
